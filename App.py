...
def get_gspread_client() -> Optional[GSpreadClient]:
    ...
    try:
        # 1. Ajustado para o nome correto do segredo no TOML: "google_sheets"
        creds_dict = dict(st.secrets["google_sheets"])

        # 2. Limpeza manual da chave para remover o UserWarning de "malformed keyfile"
        if "private_key" in creds_dict:
            # Substitui \n literal por quebra de linha real e remove espaços extras
        ...
        # Imprime o erro no console para você saber exatamente o que falhou
...
def preprocess_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[mask_empty_tipo, "TIPO"] = df.loc[mask_empty_tipo, "VALOR_NUM"].apply(
        lambda v: "Despesa" if v < 0 else "Receita"
    )
    ...
    df["year_month"] = df["DATA"].dt.to_period("M").astype(str)  # type: ignore
    return df
...
def _get_empty_fig(text: str = "Sem dados") -> go.Figure:
    ...
    fig.add_annotation(
        ...
        y=0.5,
        showarrow=False,
        font=dict(color="var(--caec-text-secondary)"),
    )
    fig.update_layout(
...
def plot_fluxo_diario(df: pd.DataFrame) -> go.Figure:
    if df.empty:
        return _get_empty_fig()
    fluxo = (
        df.groupby(df["DATA"].dt.date)["VALOR_NUM"].sum().reset_index()
    )  # pyright: ignore[reportAttributeAccessIssue]
    fluxo["DATA"] = pd.to_datetime(fluxo["DATA"])
    cores = [
        COLORS["receita"] if v >= 0 else COLORS["despesa"] for v in fluxo["VALOR_NUM"]
    ]
...
def plot_categoria_barras(
    df: pd.DataFrame, kind: str = "Receita", category_colors: Dict[str, str] = None
) -> go.Figure:
    ...
    vals = series.values
    marker_colors = (
        [category_colors.get(c, COLORS["neutral"]) for c in cats]
        if category_colors
        else [COLORS["receita"]] * len(cats)
    )
    fig = go.Figure(
        go.Bar(x=vals, y=cats, orientation="h", marker=dict(color=marker_colors))
    )
    fig.update_layout(
...
def plot_pie_composicao(
    ...
    marker_colors = (
        [category_colors.get(l, COLORS["neutral"]) for l in labels]
        if category_colors
        else None
    )
...
def plot_bubble_transacoes_categoria_y(
    df: pd.DataFrame, category_colors: Dict[str, str] = None
) -> go.Figure:
    if df.empty:
    ...
    fig = px.scatter(
        ...
        hover_data={"VALOR_FMT": True, "DATA": False},
        color_discrete_map=category_colors,
        size_max=35,
    )
    fig.update_traces(
        marker=dict(opacity=0.85, line=dict(width=0.6, color="rgba(0,0,0,0.12)"))
    )
    fig.update_layout(
...
def plot_bubble_transacoes_valor_y(
    df: pd.DataFrame, category_colors: Dict[str, str] = None
) -> go.Figure:
    if df.empty:
        return _get_empty_fig("Sem transações")
...
def plot_monthly_heatmap(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title="Heatmap Mensal de Saldo Diário",
        height=DEFAULT_CHART_HEIGHT + 40,
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
    )
...
def sidebar_filters_and_controls(df: pd.DataFrame) -> Tuple[str, Dict]:
    ...
    min_ts = (
        df["DATA"].min()
        if not df.empty
        else pd.Timestamp(datetime.today() - timedelta(days=365))
    )
    ...
    if toggle_multi:
        ...
        months = (
            ["Todos"] + sorted(df["year_month"].unique(), reverse=True)
            if not df.empty
            else ["Todos"]
        )
        selected_month = st.sidebar.selectbox("Mês (ano-mês)", months, key="sb_month")
        categories = (
            ["Todos"] + sorted(df["CATEGORIA"].unique()) if not df.empty else ["Todos"]
        )
        categories = [c for c in categories if c != ""]
        selected_category = st.sidebar.selectbox(
            "Categoria", categories, key="sb_cat_single"
        )
        filters["mode"] = "month"
        filters["month"] = selected_month
        filters["categories"] = (
            [selected_category] if selected_category != "Todos" else []
        )
    st.sidebar.markdown("---")
    ...
    st.sidebar.markdown("---")
    st.sidebar.caption(
        "Criado e administrado pela diretoria de Administração Comercial e Financeiro — by Rick"
    )
    return page, filters
...
def _sum_period(
    df: pd.DataFrame, start_dt: datetime, end_dt: datetime, tipo: str = "all"
) -> float:
...
def _kpi_delta_text_and_color(
    curr: float, prev: float, positive_is_good: bool = True
) -> Tuple[str, str]:
    ...
    pct = (
        (diff / abs(prev)) * 100
        if abs(prev) > 0.0001
        else (100.0 if abs(diff) > 0.0 else 0.0)
    )
    sign = "+" if diff >= 0 else "-"
    absdiff = abs(diff)
...
def render_kpi_cards(df_full: pd.DataFrame, df_filtered: pd.DataFrame):
    ...
    txt_rec_delta, color_rec = _kpi_delta_text_and_color(
        receita_curr, receita_prev, positive_is_good=True
    )
    txt_dep_delta, color_dep = _kpi_delta_text_and_color(
        -despesa_curr, -despesa_prev, positive_is_good=False
    )
    saldo_curr = receita_curr + despesa_curr
    saldo_prev = receita_prev + despesa_prev
    txt_saldo_delta, color_saldo = _kpi_delta_text_and_color(
        saldo_curr, saldo_prev, positive_is_good=True
    )
...
def _render_kpi_card_html(
    title: str, value: str, delta: str, value_color: str, delta_color: str
):
    arrow = "—"
...
def render_table(df: pd.DataFrame, key: str):
    ...
    st.dataframe(
        df_display[
            ["Data", "Tipo", "Categoria", "Descrição", "Valor (R$)", "Observação"]
        ],
        width="stretch",
        key=key,
        hide_index=True,
...
def main():
    ...
    if df_full.empty:
        st.warning(
            "Planilha vazia ou erro ao importar dados. Verifique a planilha/credenciais."
        )
        st.stop()
    ...
    if page == "Resumo Financeiro":
        ...
        st.plotly_chart(
            plot_saldo_acumulado(df_filtered),
            width="stretch",
            config={"displayModeBar": False},
            key="chart_saldo_line_resumo",
        )
        st.subheader("Fluxo de Caixa Diário")
        st.plotly_chart(
            plot_fluxo_diario(df_filtered),
            width="stretch",
            config={"displayModeBar": False},
            key="chart_fluxo_bar_resumo",
        )
        ...
        tab_normais, tab_avancados, tab_tabela = st.tabs(
            ["📊 Gráficos Principais", "📈 Análise Avançada", "📋 Tabela Completa"]
        )
        with tab_normais:
            col1, col2 = st.columns(2)
            with col1:
                st.plotly_chart(
                    plot_categoria_barras(
                        df_filtered, kind="Receita", category_colors=category_colors
                    ),
                    width="stretch",
                    config={"displayModeBar": False},
                    key="chart_rec_bar_comb",
                )
                st.plotly_chart(
                    plot_pie_composicao(
                        df_filtered, kind="Receita", category_colors=category_colors
                    ...
                    width="stretch",
                    config={"displayModeBar": False},
                    key="chart_pie_rec_comb",
                )
            with col2:
                st.plotly_chart(
                    plot_categoria_barras(
                        df_filtered, kind="Despesa", category_colors=category_colors
                    ),
                    width="stretch",
                    config={"displayModeBar": False},
                    key="chart_dep_bar_comb",
                )
                st.plotly_chart(
                    plot_pie_composicao(
                        df_filtered, kind="Despesa", category_colors=category_colors
        ...
        with tab_avancados:
            ...
            st.plotly_chart(
                plot_candlestick(df_filtered, freq=freq_code),
                width="stretch",
                config={"displayModeBar": False},
                key=f"chart_candlestick_{freq_code}",
            )
        with tab_tabela:
    ...
    st.markdown("---")
    st.markdown(
        "<div style='text-align:center;color:var(--caec-text-secondary);'>CAEC © 2025 — Criado e administrado pela diretoria de Administração Comercial e Financeiro — <strong>by Rick</strong></div>",
